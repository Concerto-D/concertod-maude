...
def sensor_type():
    ...
    t.set_initial_place(pl_off)
    t.set_running_place(pl_running)
    bhv_deploy = t.add_behavior("start")
    bhv_deploy.add_transition("start11", pl_off, pl_provisioned)
    bhv_deploy.add_transition("start12", pl_off, pl_provisioned)
    bhv_deploy.add_transition("start13", pl_off, pl_provisioned)
    bhv_deploy.add_transition("start2", pl_provisioned, pl_installed)
    bhv_deploy.add_transition("start3", pl_installed, pl_configured)
    bhv_deploy.add_transition("start4", pl_configured, pl_running)
...
def sensor_deploy(i):
    return Program([
        ...
        Connect(f"listener{i}", "rcv", f"sensor{i}", "rcvService"),
        Connect(f"listener{i}", "config", f"sensor{i}", "configService"),
        PushB(f"sensor{i}", "deploy", f"b{i}sens1")
    ])
